...
class Simulacion:
    ...
    def acumular_stll(self, variables_sistema, prioridad):
        if prioridad == "ALTA":
            variables_sistema["STLLA"] = variables_sistema["STLLA"] + variables_sistema["T"]
        elif prioridad == "MEDIA":
            variables_sistema["STLLM"] = variables_sistema["STLLM"] + variables_sistema["T"]
        else:
            variables_sistema["STLLB"] = variables_sistema["STLLB"] + variables_sistema["T"]

    def get_intervalo_entre_arribos(self):
        num = round(random.uniform(0.0001, 0.9999), 4)
        return -4489.54*math.log(1-num) # -4489,54*log(1-x)
    ...
    def generar_tiempo_resolucion_jr(self):
        ...
        return 9000/((1-num) ** 1.3368805) # 9000/((1-x)^(1.3368805))
    ...
    def calcular_resultados(self, variables):
        ...
        variables["PECA"] = (variables["STSA"] - variables["STAA"] - variables["STLLA"]) / variables["NT"]
        variables["PECM"] = (variables["STSM"] - variables["STAM"] - variables["STLLM"]) / variables["NT"]
        variables["PECB"] = (variables["STSB"] - variables["STAB"] - variables["STLLB"]) / variables["NT"]
